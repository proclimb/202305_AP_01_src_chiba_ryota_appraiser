...
function fnConstEditCheck() {
	tmp = form.area.value;
	if (tmp.length > 0 && !tmp.match(/^([0-9]{1,3})(\.[0-9]{1,2})?$/)) {//小数点が入力できなかったため修正
		alert('面積は3桁以内（小数点以下2桁以内）の半角数字で入力してください');
		return;
	}

	if (isLength(100, "築年", form.years)) { return; }//1.{ return; }を入れないと入力エラーのポップアップが表示された後に登録確認のポップアップが表示されるため修正　2.unctionを呼び出すスペルが間違っていて正しく呼び出せていなく更新ボタンが動作しなかったため修正
	if (isNumericLength(9, "販売予定額", form.sellPrice)) { return; } //エラーポップアップに表示する桁数を指示する数字がプログラムでは「9桁」まで入力可能なのに、ここで「8桁まで」と設定してしまっているので修正
	if (isNumericLength(9, "内装見越額", form.interiorPrice)) { return; }//スペルミスにより送信されてくるデータを受け取れなかったので更新ボタンが動作しなかったため修正
	...
	if (!fnYMDCheck("正しい現調日付", form.siteDate)) { return; }//functionのコードでtrueできた値をfalseに変換しなければ正常に動作しないため修正
	if (isLength(100, "届出用紙", form.leavingForm)) { return; }
	if (!fnYMDCheck("正しい届出期日", form.leavingDT)) { return; }//functionのコードでtrueできた値をfalseに変換しなければ正常に動作しないため修正
