...
{
    $select = "SELECT IF(SEARCHDT > '0000-00-00',DATE_FORMAT(SEARCHDT,'%Y/%m/%d'),''),ARTICLE,ADDRESS,STATION,IF(FOOT > 0,FOOT,''),"; //日付の呼び出しがyyyy-mm-ddのハイフン形式の設定だったためyyyy/mm/ddのスラッシュ形式に修正
...
{
    $sql = "UPDATE TBLSELL";
    $sql .= " SET DEL = -1"; //削除の更新を1で行っていたため-1にし画面表示させないよう修正
    $sql .= ",UPDT = CURRENT_TIMESTAMP";
