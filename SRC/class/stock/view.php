...
?>
	...
	<form name="form" id="form" action="index.php" method="post">
		...
		<div class="search">
			<table border="0" cellpadding="2" cellspacing="0">
				...
				<tr>
					...
					<td>
						<?php
						for ($i = 0; $i < 4; $i++) { //アルファベットの4番目までを表示させるよう修正
						?>
				...
				<tr>
					...
					<td>
						<?php
						for ($i = 0; $i < 5; $i++) { //アルファベットの5番目までを表示させるよう修正
						?>
		...
		$sql = fnSqlStockList(0, $param); //ページカウントをしない状態だったのでページ表示がうまくいかなかったため修正
...
?>
	...
	<form name="form" id="form" action="index.php" method="post"><!--備考の1000文字を登録しようとするとURLが長すぎてエラーになるし、安全性に欠けているので修正-->
		...
		<a href="javascript:form.act.value='stockSearch';form.submit();"><img src="./images/btn_return.png" /></a><!--戻るボタンのvalueが登録用の値だったため修正-->
		<?php
		if ($param["stockNo"]) {
		?>
