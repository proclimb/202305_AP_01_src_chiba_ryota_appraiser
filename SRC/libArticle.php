...
function subArticle()
{
	...
	$sDel         = htmlspecialchars($_REQUEST['sDel']); //検索項目にクロスサイトスクリプティングができるワードが入っている状態で検索すると画面表示に異常が発生してしまうため修正
	...
	<form name="form" id="form" action="index.php" method="post">
		...
		<div class="search">
			<table border="0" cellpadding="2" cellspacing="0">
				...
				<tr>
					<th>物件名</th>
					<td><input type="text" name="sArticle" value="<?php print $sArticle ?>" size="50" /></td> <!--valueの変数$sRoomが$sArticleでないと正常に検索できないため修正-->
					<th>キーBox番号</th>
					<td><input type="text" name="sKeyBox" value="<?php print $sKeyBox ?>" size="30" /></td>
				</tr>
				<tr>
					<th>部屋番号</th>
					<td><input type="text" name="sRoom" value="<?php print $sRoom ?>" size="30" /></td> <!--valueに変数が設定されたいなかったため修正-->
					<th>3Dパース</th>
					<td><input type="text" name="sDrawing" value="<?php print $sDrawing ?>" size="30" /></td>
				</tr>
				<tr>
					<th>鍵場所</th>
					<td><input type="text" name="sKeyPlace" value="<?php print $sKeyPlace ?>" size="30" /></td><!--valueに変数のスペルが間違いだったため修正-->
					<th>営業担当者</th>
	...
	<div class="list">
		<table border="0" cellpadding="5" cellspacing="1">
			...
			while ($row = mysqli_fetch_array($res)) {
				$articleNo   = htmlspecialchars($row["ARTICLENO"]); //検索結果にクロスサイトスクリプティングができるワードが含まれていると画面表示に異常が発生してしまうので修正
...
function subArticleEdit()
{
	...
	$sDel         = htmlspecialchars($_REQUEST['sDel']); //検索欄にクロスサイトスクリプティングに関するワードが入った状態で新規登録・更新画面に遷移すると画面表示に異常が発生するため修正
	$sArticle     = htmlspecialchars($_REQUEST['sArticle']);
	$sRoom        = htmlspecialchars($_REQUEST['sRoom']);
	$sKeyPlace    = htmlspecialchars($_REQUEST['sKeyPlace']);
	$sArticleNote = htmlspecialchars($_REQUEST['sArticleNote']);
	...
	if ($articleNo) {
		...
		$article     = htmlspecialchars($row["ARTICLE"]); //登録してある情報にクロスサイトスクリプティングに関するワードが含まれていると更新時の画面表示に異常が発生するため修正
		$room        = htmlspecialchars($row["ROOM"]);
		$keyPlace    = htmlspecialchars($row["KEYPLACE"]);
